        ...
        if (not usrdata['status']=='None'): #Successfully query data from website
            ...
            if(not usrdata['type']=='lim'):
                ...
                if(usrdata['type']=='vis'):
                    ...
                    primRole = discord.utils.get(guild.roles,name= FACILITY_ID[0]+ " Controller")
            ...
            if live:
                if(not onLIVE):
                    embed = discord.Embed(colour=0x600080, title='LIVE Mode Deactivated', url=site_url[0])
                    embed.add_field(name='Successfully Deactivate LIVE Mode',
                            ...
                            inline=False)
                    embed.set_footer(text='Maintained by the v' + FACILITY_ID[0] + ' Web Services Team')
                else:
                    embed = discord.Embed(colour=0x600080, title='LIVE Mode Activated', url=site_url[0])
                    embed.add_field(name='Follow The Policies',
                            value='Please remember that your stream is subject to the Facility Operations Policy (https://vats.im/' + FACILITY_ID[0] + '/fop) and VATSIM Code of Conduct (https://vats.im/coc) and to abide by all rules enforcing streams on the Discord and live network respectively.',
                            inline=False)
                ...
                embed = discord.Embed(colour=0x32cd32, title='Roles Synced', url=site_url[0])
                embed.add_field(name='Successfully Synced Roles',
                        value='Thank you for joining the Virtual ' + FACILITY_NAME[0] + ' Discord. Your roles & nickname have been synced according to our ARTCC site.',
                        inline=False)
                embed.set_footer(text='Maintained by the v' + FACILITY_ID[0] + ' Web Services Team')
            ...
            embed = discord.Embed(colour=0xf70d1a, title='Account Not Linked', url=site_url[0])
            embed.add_field(name='Please Link Account',
                    value='If you have not already done so please link your Discord account to our ARTCC site (' + site_url[0] + '), and re-execute the command.',
                    inline=False)
            embed.set_footer(text='Maintained by the v' + FACILITY_ID[0] + ' Web Services Team')
            await user.send(embed=embed)
            return 0
...
async def spontaneous_embed(message):
    embed = discord.Embed(colour=0x2664D8, title='Spontaneous Training', url=site_url[0])
...
async def spontaneous(message,command,guild):
    if (message.channel.id == int(SP_Channel_ID)):
        content = message.content.replace(prefix[0]+command,"")
        limit = []
        ...
        if limit:
            ...
            embed.set_footer(text = 'Maintained by the v'+FACILITY_ID[0]+' Web Services Team and Training Department')
        else:
            embed = discord.Embed(
                color=0xFFCC00,
                title="Spontaneous Training Available",
...
async def activity(message,client):
    ...
    query = message.content.replace(prefix[0]+'activity ',"")
    query = query.replace(" ",",")
    users = webQuery(site_url[0] + '/api/data/bot/search/?ois='+query,site_token[0])
    for usrid in users:
        if (usrid):
...
async def removeroles(message,guild):
    count = 0
    query = message.content.replace(prefix[0]+'removeroles ',"")
    query = query.replace(" ",",")
    users = webQuery(site_url[0] + '/api/data/bot/search/?ois='+query,site_token[0])
    for usrid in users:
        if (usrid):
...
async def addEvent(message,guild):
    eventid = message.content.lower().replace(prefix[0]+'addevent ',"")
    query = site_url[0]+'/api/data/bot/event.php?event_id='+str(eventid)
...
async def debugMsg(message):
    ...
    embed.add_field(name='Debug Message',
                value=message.content.lower().replace(prefix[0]+'debugmsg ',""),
                inline=False)
